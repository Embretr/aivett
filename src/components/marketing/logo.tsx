...
export function Logo({ className, wordmarkOnly }: { className?: string; wordmarkOnly?: boolean }) {
  return (
    <Link href="/" className={cn("flex items-center gap-2.5 shrink-0", className)}>
      {!wordmarkOnly && (
        ...
        >
          ...
          {/* Spark mark — six-pointed asterisk representing insight/vett */}
      ...
      <span className="text-[1.05rem] font-bold tracking-tight text-gray-950 select-none">
        AI<span className="text-violet-700">vett</span>
      </span>
    </Link>
  )
}
